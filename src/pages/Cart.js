...
import {addPizzaToCart, minusPizzaFromCart, removePizzaFromCart, clearCart} from '../redux/actions/cartAction'
import {Link} from 'react-router-dom'

function Cart() {
  const dispatch = useDispatch()
  const cart = useSelector(({cart}) => cart)
  const cartItems = Object.values(cart.items).flat(2)
  const handleAddPizza = (pizza) => {
    dispatch(addPizzaToCart(pizza))
  }
  const handleMinusPizza = (pizza) => {
    dispatch(minusPizzaFromCart(pizza))
  }
  const handleRemovingPizza = (pizza) => {
          ...
          <div className="content__items-cart">
            {
              ...
              cartItems.map((pizza, i) => 
                <CartItem key={`${pizza.id}_${i}`} pizza={pizza}
                          onAddPizza = {handleAddPizza} onMinusPizza = {handleMinusPizza}
                          onRemovePizza = {handleRemovingPizza}
                />
              )
            }
